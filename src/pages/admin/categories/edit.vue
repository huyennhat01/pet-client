<template>
  <form @submit.prevent="updateCategories()" class="h-100">
    <a-card title="Chỉnh sửa thể loại sách" class="h-100">
      <div class="row">
        <div class="col-12">
...
export default defineComponent({
  ...
  created() {
    document.title = "Chỉnh sửa thể loại sách";
    this.getCategories();
  },
  methods: {
    async getCategories() {
      try {
        ...
        if (res.status == 200) {
          ...
          this.categories.status = data.status_id;
        ...
        try {
          ...
          if (res.status == 200) {
            setTimeout(() => {
              ...
              this.router.push({ name: "admin-list-categories" });
            }, 500);
          }
        } catch (error) {
          if (error) {
