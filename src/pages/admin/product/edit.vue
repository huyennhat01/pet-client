<template>
  <form @submit.prevent="updateProduct()" class="product h-100">
    <a-card title="Chỉnh sửa sản phẩm" class="h-100">
      <div class="row">
  ...
  methods: {
    ...
    validate() {
      ...
      if (!this.product.name) {
        this.errors.name = "Tên sản phẩm không thể bỏ trống!";
        inValid = false;
      } else if (
        this.product.name.length < 4 ||
        this.product.name.length > 5000
      ) {
        this.errors.name = "Độ dài tên sản phẩm không hợp lệ!";
        inValid = false;
      }
      ...
      if (!this.product.author) {
        this.errors.author = "Mô tả ngắn không thể bỏ trống!";
        inValid = false;
      } else if (
      ...
      if (!this.product.categories) {
        this.errors.categories = "Danh mục không được để trống!";
        inValid = false;
      }
    ...
    async updateProduct() {
      if (this.validate()) {
        try {
          ...
          if (res.status == 200) {
            setTimeout(() => {
              this.router.push({ name: "admin-list-products" });
              message.success({
...
@media (max-width: 576px) {
  .product .preview-image-groups .image-container:nth-child(n + 4) {
    display: none;
  }
}
@media (min-width: 576px) {
  .product .preview-image-groups .image-container:nth-child(n + 5) {
